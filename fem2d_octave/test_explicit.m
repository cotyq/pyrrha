function [OK] = test_explicit()
    fd = fopen('results.txt', 'a');
    module = 'fem2d_heat_explicit.m';
    fprintf('Validando %s...\n', module);
    ...
    [dt] = fem2d_heat_explicit_delta_t(xnode,icone,model); %#ok<NODEF>
    model.maxit = 50;
    model.tol = 1e-5;
    model.verbose = 0;
    
    [PHI1, Q1] = fem2d_heat_explicit(K,C,F,xnode,icone,model,dt);
    ...
    try
        [PHI2, Q2] = temp_fem2d_heat_explicit(K,C,F,xnode,icone,model,dt);
    catch
        message = 'Se produjo un error durante la ejecución de fem2d_heat_explicit.\n';
        PHI2 = [];
        Q2 = [];
